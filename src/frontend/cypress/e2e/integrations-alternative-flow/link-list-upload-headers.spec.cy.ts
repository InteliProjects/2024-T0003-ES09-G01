describe('Link Lists Endpoints', () => {
    ...
    it('Teste deve falhar pelo envio com os headers errados.', () => {
      ...
      cy.fixture('../fixtures/file_model - file_model.csv', 'utf-8').then((fileContent) => {
        cy.request({
          ...
          cy.on('uncaught:exception', (err) => {
            expect(err.message).to.equal('Os headers do CSV não correspondem aos esperados ou estão fora de ordem.');
            return false;
          });
        });
      });
